(() => {
  ...
  function resolveGroundPosition(position) {
    ...
    for (let probe = position.z + 50; probe >= -50; probe -= 10) {
      try {
        const result = mp.game.gameplay.getGroundZFor3dCoord(position.x, position.y, probe, 0, false);
        if (Array.isArray(result) && result[0]) {
          return new mp.Vector3(position.x, position.y, result[1] + 1);
        }
  ...
  function getWaypointPosition() {
    try {
      if (mp.game.ui && typeof mp.game.ui.getFirstBlipInfoId === "function") {
        const blip2 = mp.game.ui.getFirstBlipInfoId(WAYPOINT_BLIP_ID);
        if (blip2 && (!mp.game.ui.doesBlipExist || mp.game.ui.doesBlipExist(blip2))) {
          const coord = mp.game.ui.getBlipInfoIdCoord(blip2);
          if (coord) {
            return new mp.Vector3(coord.x, coord.y, coord.z || 0);
          }
        }
      }
  ...
  function resolveWaypointGround(position) {
    for (let z = 1e3; z >= -80; z -= 25) {
      try {
        mp.game.streaming.requestCollisionAtCoord(position.x, position.y, z);
        const result = mp.game.invoke("0x8BDC7BFC57A81E76", position.x, position.y, z, 0, false);
        if (Array.isArray(result) && result[0]) {
          return new mp.Vector3(position.x, position.y, result[1] + 1);
        }
        if (typeof result === "number" && Number.isFinite(result) && result !== 0) {
          return new mp.Vector3(position.x, position.y, result + 1);
        }
      } catch (error) {
        break;
      }
    }
    return new mp.Vector3(position.x, position.y, 80);
  }
  ...
  mp.events.add("render", () => {
    ...
    const fastMultiplier = mp.keys.isDown(KEY_SHIFT) ? 3 : 1;
    const slowMultiplier = mp.keys.isDown(KEY_CTRL) ? 0.35 : 1;
    ...
    const upMovement = mp.keys.isDown(KEY_SPACE) ? 0.55 : 0;
    const downMovement = mp.keys.isDown(KEY_CTRL) ? 0.55 : 0;
    ...
    state.camera.setRot(
      rotation.x + rightAxisY * -5,
      0,
      rotation.z + rightAxisX * -5,
      2
    );
