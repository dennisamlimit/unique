...
function hasRequiredAdminLevel(): boolean {
  try {
    const level = mp.players.local.getVariable("ADMIN_LEVEL");
    return Number.isFinite(level) && (level as number) >= 2;
  } catch (error) {
    return false;
  }
}
...
function normalize(vector: Mp.Vector3): Mp.Vector3 {
...
function cross(v1: Mp.Vector3, v2: Mp.Vector3): Mp.Vector3 {
...
function resolveGroundPosition(position: Mp.Vector3): Mp.Vector3 {
  ...
  for (let probe = position.z + 50.0; probe >= -50.0; probe -= 10.0) {
    try {
      ...
      if (Array.isArray(result) && result[0]) {
        return new mp.Vector3(position.x, position.y, (result as number[])[1] + 1.0);
      }
...
function getWaypointPosition(): Mp.Vector3 | null {
  try {
    if (mp.game.ui && typeof (mp.game.ui as any).getFirstBlipInfoId === "function") {
      const blip = (mp.game.ui as any).getFirstBlipInfoId(WAYPOINT_BLIP_ID);
      if (blip && (!(mp.game.ui as any).doesBlipExist || (mp.game.ui as any).doesBlipExist(blip))) {
        const coord = (mp.game.ui as any).getBlipInfoIdCoord(blip);
        if (coord) {
    ...
    if (blip && mp.game.invoke("0xA6DB27D19ECBB7DA", blip)) {
      const coord = mp.game.invoke("0xFA7C7F0AADF25D09", blip) as Mp.Vector3;
      return new mp.Vector3(coord.x, coord.y, coord.z || 0.0);
    }
  } catch (error) {
    return null;
...
function resolveWaypointGround(position: Mp.Vector3): Mp.Vector3 {
  for (let z = 1000.0; z >= -80.0; z -= 25.0) {
    try {
      (mp.game as any).streaming.requestCollisionAtCoord(position.x, position.y, z);
      const result = mp.game.invoke("0x8BDC7BFC57A81E76", position.x, position.y, z, 0.0, false);
...
function teleportToWaypoint(): void {
...
function startNoclip(): void {
  ...
  state.camera = (mp.cameras as any).new("adminNoclip", position, rotation, 45);
  state.camera.setActive(true);
  (mp.game.cam as any).renderScriptCams(true, false, 0, true, false);
...
function stopNoclip(snapToGround: boolean): void {
  const localPlayer = mp.players.local as any;
  ...
  (mp.game.cam as any).renderScriptCams(false, false, 0, true, false);
...
function toggleNoclip(): void {
  ...
  if (state.active) {
    stopNoclip((mp.keys as any).isDown(KEY_SPACE));
    return;
  }
...
mp.events.add("render", () => {
  ...
  const fastMultiplier = (mp.keys as any).isDown(KEY_SHIFT) ? 3.0 : 1.0;
  const slowMultiplier = (mp.keys as any).isDown(KEY_CTRL) ? 0.35 : 1.0;
  ...
  const rightAxisX = (mp.game.controls as any).getDisabledControlNormal(0, 220);
  const rightAxisY = (mp.game.controls as any).getDisabledControlNormal(0, 221);
  const leftAxisX = (mp.game.controls as any).getDisabledControlNormal(0, 218);
  const leftAxisY = (mp.game.controls as any).getDisabledControlNormal(0, 219);
  ...
  const upMovement = (mp.keys as any).isDown(KEY_SPACE) ? 0.55 : 0.0;
  const downMovement = (mp.keys as any).isDown(KEY_CTRL) ? 0.55 : 0.0;
  ...
  (mp.players.local as any).position = new mp.Vector3(cameraPosition.x, cameraPosition.y, cameraPosition.z);
  (mp.players.local as any).heading = state.camera.getRot(2).z;
});
