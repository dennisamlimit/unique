...
function getStreetInfo(position: Mp.Vector3) {
  try {
    const streetData = mp.game.pathfind.getStreetNameAtCoord(position.x, position.y, position.z, 0, 0);
    if (!streetData) {
      return { street: "", crossing: "" };
    }

    const street = streetData.streetName ? (mp.game.ui.getStreetNameFromHashKey(streetData.streetName) || "") : "";
    const crossing = streetData.crossingRoad ? (mp.game.ui.getStreetNameFromHashKey(streetData.crossingRoad) || "") : "";
    return { street, crossing };
  } catch (error) {
    return { street: "", crossing: "" };
  }
...
function updateHud(): void {
  ...
  try {
    ...
    const streetInfo = getStreetInfo(position);
    const heading = typeof mp.players.local.getHeading === "function"
      ? mp.players.local.getHeading()
      : 0;

    executeHud(`window.hudApp && window.hudApp.updateLocation(${JSON.stringify(zoneName)}, ${JSON.stringify(streetInfo.street)}, ${JSON.stringify(streetInfo.crossing)}, ${JSON.stringify(getHeadingLabel(heading))});`);
...
mp.events.add("client:adminJail:show", (...args: unknown[]) => {
  ...
  executeHud(`window.hudApp && window.hudApp.showJail(${JSON.stringify(rawData || "{}")});`);
});
