...
const UNIQUE_PURPLE: [number, number, number, number] = [217, 70, 239, 220];
const TEXT_WHITE: [number, number, number, number] = [255, 255, 255, 190];
...
function isAllowedToUseDl(showMessage: boolean): boolean {
...
function getStreamedVehicles(callback: (vehicle: Mp.Vehicle) => void): void {
  try {
...
function getVehicleName(vehicle: Mp.Vehicle): string {
  try {
...
function worldToScreen(position: Mp.Vector3): { x: number; y: number } | null {
  try {
...
function drawVehicleInfo(vehicle: Mp.Vehicle): void {
  try {
    ...
    const remoteId = Number.isFinite((vehicle as any).remoteId) ? (vehicle as any).remoteId : "-";
...
mp.events.add("playerCommand", (...args: unknown[]) => {
  const [command] = args;
