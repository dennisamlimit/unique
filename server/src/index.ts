...
function buildJailPayload(reason: string, releaseAt: number, admin = "Unbekannt") {
  return JSON.stringify({
    admin,
    reason,
...
function buildPlayerHudTicketPayload(ticket: Awaited<ReturnType<TicketService["getOpenTicketByAccountId"]>>) {
  return JSON.stringify({ visible: false, ticket: null });
}
...
function jailPlayer(target: any, durationMs: number, reason: string, admin = "Unbekannt") {
  clearAdminJailTimer(target.id);
  ...
  emitClient(target, "client:adminJail:show", buildJailPayload(reason, releaseAt, admin));
...
async function bootstrap() {
  ...
  mp.events.add("playerQuit", (player: any) => {
    ...
    const accountId = Number(getVar(player, "ACCOUNT_ID", 0));
    const isAdmin = hasAdminAccess(player);
    const playerName = getPlayerName(player);
    void (async () => {
      if (accountId > 0 && isAdmin) {
        await tickets.releaseClaimsForAdmin(accountId, playerName);
      }
      if (accountId > 0) {
    ...
    })().catch((error) => logError("playerQuit ticket sync failed", error));
  });
  ...
  setInterval(() => {
    void (async () => {
      await tickets.releaseInactiveClaims(15);
      await tickets.autoCloseExpiredTickets(30);
      await syncTicketState();
    })().catch((error) => logError("ticket sync interval failed", error));
  }, 15 * 1000);
  ...
  mp.events.add("server:admin:getCommandList", (player: any) => {
    void (async () => {
        ...
        await sendAdminTicketData(player);
  ...
  mp.events.add("server:tickets:requestMine", (player: any) => {
    void sendPlayerTicketData(player).catch((error) => logError("request my tickets failed", error));
  });
